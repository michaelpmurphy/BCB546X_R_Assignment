...
###MPMURPHY COMMENTS > Ah yes, I pulled the source files from a local folder on my computer (where I'd originally saved the files for the Unix assignment).  I should have pulled them directly from the GitHub repository to make my code reproducible on other machines.
...
# line 61: 
#   > colnames(snp_known)[1] <- "SNP_ID"
###MPMURPHY COMMENTS > That appears to be a relic of the specific `snp_position` file you pulled from your repository. The original file had the column name `SNP_ID` so I never had to modify it in my files.
...
###MPMURPHY COMMENTS > I really like that suggestion!!!  Originally I was envisioning just counting the number of true SNPs on each chromosome (i.e. multiplying each SNP site by the number of groups that showed heterozygosity at that site) without differentiating by group, and then breaking down the TOTAL contribution from each group in the accompanying table.  But adding the group fill to the graph itself is a very nice touch!
...
###MPMURPHY COMMENTS > Absolutely, that's exactly how I'd wanted to make it look to begin with!! I simply didn't realize it was possible to use the `fill` option for a variable whose value was constant within each bar. Thanks!!
```
